(() => {
    ...
    const openNew = () => {
        titleEl.textContent = "Yeni Gider Kategorisi";
        idEl.value = "0";
        nameEl.value = "";
    ...
    const openEdit = (x) => {
        titleEl.textContent = "Gider Kategorisi Düzenle";
        idEl.value = x.id;
        nameEl.value = x.name;
        errEl.classList.add("d-none");
        modal.show();
    ...
    const load = async () => {
        const data = await app.get("/DebtTypes/List");
        body.innerHTML = "";
    ...
    body.addEventListener("click", async (e) => {
        ...
        if (act === "detail") {
            ...
            window.location.href = `/DebtTypes/Details/${id}`;
            return;
        }
        ...
        if (act === "del") {
            ...
            try {
                await app.postJson("/DebtTypes/Delete", id);
                await load();
            } catch (err) {
    ...
    btnSave.addEventListener("click", async () => {
        ...
        try {
            await app.postJson("/DebtTypes/Upsert", payload);
            modal.hide();
            await load();
        } catch (err) {
